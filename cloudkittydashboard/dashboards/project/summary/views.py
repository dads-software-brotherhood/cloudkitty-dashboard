...
class CostRepartitionTab(tabs.Tab):
    name = "This month"
    slug = "this_month"
    template_name = 'project/summary/this_month.html'

    def get_context_data(self, request, **kwargs):
        total = api.cloudkittyclient(request).reports.get_total(tenant_id=request.user.tenant_id) or 0.00
        ...
        date_start = datetime.datetime(today.year, today.month, 1)
        date_end = datetime.datetime(today.year, today.month, day_end)
        return {'repartition_data': parsed_data, 'date_start': date_start, 'date_end': date_end, 'total': total}


class SummaryTabs(tabs.TabGroup):
    slug = "summary_tabs"
...
class IndexView(tabs.TabbedTableView):
    tab_group_class = SummaryTabs
    template_name = 'project/summary/index.html'
